...
void USART1_IRQHandler(void){ //串口1入口函数
	...
	if(USART_GetITStatus(USART1,USART_IT_RXNE)!=Bit_RESET){	//判断是否进入接收中断	
		k=USART_ReceiveData(USART1);
		k++;
		USART_SendData(USART1,k);
		while(USART_GetFlagStatus(USART1,USART_FLAG_TXE)==Bit_RESET); // 等待发送完成
		// GPIO_WriteBit(GPIOA,GPIO_Pin_2,(BitAction)(Bit_SET-GPIO_ReadOutputDataBit(GPIOA,GPIO_Pin_2))); 
	}
}


void USART2_IRQHandler(void){ 	   //串口2入口函数
	static u8 k;
	USART_ClearFlag(USART2,USART_FLAG_TC);//清空发送完成标志位
	if(USART_GetITStatus(USART2,USART_IT_RXNE)!=Bit_RESET){	//判断是否进入接收中断	
		k=USART_ReceiveData(USART2);
		GPIO_SetBits(GPIOG,GPIO_Pin_3);
		delay_ms(1);
		USART_SendData(USART2,k);
		while(USART_GetFlagStatus(USART2,USART_FLAG_TXE)==Bit_RESET); // 等待发送完成
		// GPIO_WriteBit(GPIOA,GPIO_Pin_2,(BitAction)(Bit_SET-GPIO_ReadOutputDataBit(GPIOA,GPIO_Pin_2))); 
		delay_ms(2);
